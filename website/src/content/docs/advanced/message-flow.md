...
- `message.decode(gun_message)` turns a raw Gun mailbox message into a typed `Envelope(connection:, stream:, message:)`, pairing the decoded `Message` with the connection and stream Gun named for it. Only the mailbox tuples Gun sends to the process that owns the stream (`gun_response`, `gun_data`, `gun_inform`, `gun_trailers`, `gun_push`, `gun_upgrade`, `gun_ws`) carry that identity, so those are the only shapes `decode` accepts; the shorter terms `gun:await/3` returns have no identity to preserve and are rejected with `DecodeError`. Use `decode` when your own Erlang FFI receives raw Gun messages outside Gluegun's helpers — for example a process that owns several concurrent streams (HTTP/2 requests and server pushes sharing one mailbox) needs the returned `Envelope` to attribute each message to the right stream instead of assuming messages arrive for whichever stream it currently has in mind. For `Push` messages, `Envelope`'s `stream` field is the *existing* stream that received the push notification, not the new stream carried inside `message.Push`.
